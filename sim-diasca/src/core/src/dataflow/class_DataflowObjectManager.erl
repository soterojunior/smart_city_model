...
% The dataflow object manager is a base (possibly overridden) class, common to
% all actual managers for a given set of types of dataflow objects (they are
% instances either directly of that class, or from child classes thereof).
%
% For example, if a simulation involves buildings in the form of dataflow
% objects, these instances could be managed by a BuildingManager that may
% inherit (probably directly) from the current class, if some domain-specific
% handling of dataflow objects is needed (ex: to manage specific associations);
% otherwise this BuildingManager may simply be directly an instance of the
% current class.
%
% If relevant, a given object manager may take care of multiple types of objects
% (ex: a manager may manage the building instances, but also the ones of
% dwellings, households, lifts, etc.).
%
% Each object manager is directly linked to the world manager.
%
% Each object manager is a singleton and registers itself globally under its
% name - which is, conventionally, its actual classname (ex:
% 'class_BuildingManager' or, if needing more clarity,
% 'class_BuildingObjectManager'). It registers as well to the world manager.
%
% As an object manager may have to create dataflow objects at runtime (for the
% types of objects it is in charge of), it must itself be a (simulation) actor.
%
...
-define( wooper_construct_parameters, ActorSettings, Name, ManagedObjectTypes,
...
-define( wooper_method_export, onFirstDiasca/2,
		 createInitialObjectInstance/4, createInitialObjectInstances/4,
		 applyChangeset/3, onActorCreated/4, onAssociationEstablished/3,
		 onBinaryAssociationEstablished/3, onDisassociationPerformed/3,
		 onAttributeUpdatePerformed/3, onDestructionTriggered/3 ).
...
% When applying a changeset, all first level events are in turn (in the same
% diasca) applied. Then they are stored (in the triggered_events attribute, with
% a tick-based timestamp) so that, on completion, their induced events can be
% applied in turn (this is true for creations and destructions, that may last
% during an unbounded number of diascas, and also for "shorter", constant-time,
...
% the suspended state, to prevent they get activated while the dataflow is still
% being modified.
%
...
-type object_table() :: table:table( dataflow_object_type(), [ object_pid() ] ).
...
%
-type completed_event_info() :: { event_id(),
								  class_WorldManager:completion_extra_info() }.
...
-define( wooper_static_method_export,
		 create_default_managers/3, create_default_managers/4,
		 create_specific_managers/3, create_specific_managers/4,
		 create_initial_object/4, create_initial_objects/4,
...
% Definition of an object manager defined by default (i.e. with no specific
% construction parameter), based only on its name and a list of the types of
% dataflow objects it will manage.
%
...
% Attributes that are specific to an object manager instance are:
%
% - object_table :: object_table() is an associative table whose keys are the
% types of objects that are managed by this instance and whose values are
% (supposedly exhaustive) lists of instances of that type (these instances are
% only referenced, not owned, as their life cycle is to be managed by their
% dataflow)
%
% - world_manager_pid :: world_manager_pid() is the PID of the world manager
%
% - dispatch_table :: table:table( object_manager_pid(), [ world_event() ] ) is
% a table associating to an object manager a list of the world events whose
% application was dispatched to it
%
% - triggered_events :: [ world_event() ] is a list of the (time-stamped, to
% detect unhandled events) world events whose application is triggered yet not
% completed; once done, their completion will be reported to the world manager
%
% - completed_event_infos :: [ completed_event_info() ] is a list of the
% information about the world events that have been completed, yet not reported
% yet to the world manager
%
% - injected_events :: [ world_event() ] is a list of the world events that the
% processing of another event led to injec (ex: if a child class of this class
% processed the creation of a car, it may inject 4 creation events for each of
...
-spec construct( wooper:state(), class_Actor:actor_settings(),
				 wooper:class_name() | { wooper:class_name(),
							class_TraceEmitter:emitter_categorization() },
				 [ dataflow_object_type() ], world_manager_pid(),
				 load_balancer_pid(),
...
construct( State, ActorSettings, Name, ManagedObjectTypes, WorldManagerPid,
	...
	?send_trace_fmt( ActorState, "Managing from now the objects of classes ~p.",
					 [ ManagedObjectTypes ] ),

	% All object managers register themselves that way:
...
prepare_for_objects( ObjectTypes, State ) ->
	...
	case class_DataflowBlock:declare_static_information_for( ObjectTypes )
		...
		ok ->
			?info_fmt( "All semantics and types for objects ~p successfully "
...
onFirstDiasca( State, _CallerPid ) ->
	...
	?void_fmt( "Created ~s.", [ to_string( State ) ] ),
...
createInitialObjectInstance( State, ObjectType, DataflowPid,
	...
	?void_fmt( "Creating an initial instance of object type '~s', associated "
	...
	NewObjectTable = table:appendToEntry( _K=ObjectType, ObjectPid,
										  ?getAttr(object_table) ),

	NewState = setAttribute( State, object_table, NewObjectTable ),

	?wooper_return_state_result( NewState, ObjectPid ).
...
createInitialObjectInstances( State, ObjectType, DataflowPid,
	...
	?void_fmt( "Creating ~B initial instances of object type '~s', associated "
	...
	ObjectList = table:getEntry( _K=ObjectType, ObjectTable ),

	NewObjectTable = table:addEntry( ObjectType, ObjectPidList ++ ObjectList,
									 ObjectTable ),

	NewState = setAttribute( State, object_table, NewObjectTable ),

	?wooper_return_state_result( NewState, ObjectPidList ).
...
applyChangeset( State, Changeset, SendingActorPid ) ->
	...
	?wooper_return_state_only( AppliedState ).
...
apply_world_events( _WorldEvents=[ CreationEvent=#creation_event{
	...
	ActorName = case ExternalID of
		...
		undefined ->
			...
			?error_fmt( "Received a creation event with no external identifier:"
						" ~s.", [ dataflow_support:world_event_to_string(
									CreationEvent ) ] ),
	...
	case lists:member( ObjectType, ManagedTypes ) of
		...
		true ->
			...
			?void_fmt( "Object manager ~p creating a '~s' instance from "
						"following construction parameters:~n~p.",
						[ self(), ObjectType, ActualConstructParams ] ),

			% ?void_fmt( "Object manager ~p creating a '~s' instance.",
			%			[ self(), ObjectType ] ),
...
apply_world_events( _WorldEvents=[ DestructionEvent=#destruction_event{
	...
	% Here we have an destruction event, yet not the corresponding PID, that
	% must be obtained first (object expected to be already existing; direct
	% message used as access to an immutable value):

	?getAttr(identification_server_pid) ! { getBlockPID,
											[ ExternalID ], self() },
...
apply_world_events( _WorldEvents=[ DestructionEvent=#destruction_event{
	...
	?trace_fmt( "Requesting the destruction of dataflow object ~p.",
				[ ObjectPid ] ),
...
apply_world_events( _WorldEvents=[ UpdateEvent=#update_event{
	...
	% used as access to an immutable value):
...
apply_world_events( _WorldEvents=[ UpdateEvent=#update_event{
	...
	% remove empty evenst from the updates
	...
	case lists:member( SourceType, ManagedTypes ) andalso
		...
		true ->
			...
			?void_fmt( "Object manager ~p taking in charge the association of "
					   "the ~s instance named '~s' to the ~s named '~s'.",
			...
			UpdatedBinAssocEvent = BinAssocEvent#binary_association_event{
									 source_object_pid=SourcePid,
									 target_object_pid=TargetPid },
...
apply_world_events( _WorldEvents=[ Event | _T ], _ManagedTypes, State ) ->
	...
	?error_fmt( "Error, world event '~p' not handled by the generic "
...
% classname of that created actor and of the parameters which were specified for
% its creation
%
% (actor oneway)
%
...
onActorCreated( State, CreatedObjectPid,
	...
	?void_fmt( "Recording the completion of the creation of the dataflow object"
			   " instance named '~s' (PID: ~w) of type '~s', created at "
	...
	case ?getAttr(identification_server_pid) of
		...
		IdPid ->
			...
			wooper:execute_request( declareIdentifierAssociation,
				[ CreatedObjectPid, BinActorName ], IdPid,
				identifier_association_declared )

	end,

	% May create a new entry for this object type:
	NewObjectTable = table:appendToEntry( _K=ObjectType, CreatedObjectPid,
										  ?getAttr(object_table) ),
	...
	{ CreationEvent, OtherTriggeredEvents } = find_creation_event(
									 CreatedActorTag, TriggeredEvents ),
...
% Parameters are:
%
% - EventId: the identifier of the corresponding completed association event
%
% (actor oneway)
%
...
onAssociationEstablished( State, EventId, _SendingActorPid ) ->
	...
	{ BinAssocEvent, OtherTriggeredEvents } = dataflow_support:find_event_by_id(
												EventId, TriggeredEvents ),
	...
	?wooper_return_state_only( PostState ).
...
% Parameters are:
%
% - EventId: the identifier of the corresponding completed binary association
% event
%
% (actor oneway)
%
...
% Parameters are:
%
% - EventId: the identifier of the corresponding completed disassociation event
%
% (actor oneway)
%
...
onDisassociationPerformed( State, EventId, _SendingActorPid ) ->
	...
	{ DisassocEvent, OtherTriggeredEvents } = dataflow_support:find_event_by_id(
												EventId, TriggeredEvents ),
...
% Parameters are:
%
% - EventId: the identifier of the corresponding completed attribute update
% event
%
% (actor oneway)
%
...
% Parameters are:
%
% - EventId: the identifier of the corresponding completed destruction event
%
% (actor oneway)
%
...
onDestructionTriggered( State, EventId, DestructedObjectPid ) ->
	...
	case ?getAttr(identification_server_pid) of
		...
		IdPid ->
			wooper:execute_request( removeIdentifierAssociation,
									[ DestructedObjectPid ], IdPid,
	...
	{ #destruction_event{
	  ...
	  OtherTriggeredEvents } = dataflow_support:find_event_by_id( EventId,
														TriggeredEvents ),
	...
	SameTypeObjects = table:getEntry( DestructedObjectType, ObjectTable ),

	ShrunkTypeObjects = list_utils:delete_existing( DestructedObjectPid,
													SameTypeObjects ),

	NewObjectTable = table:addEntry( DestructedObjectType, ShrunkTypeObjects,
									 ObjectTable ),
...
manage_post_event( _RemainingTriggeredEvents=[], CompletedEventInfos, State ) ->
	...
	?void_fmt( "Reporting ~B completed events (identifiers: ~w) "
				"to the world manager, and injecting following changeset: ~s",
...
manage_post_event( RemainingTriggeredEvents, CompletedEventInfos, State ) ->
	...
	?void_fmt( "Still ~B triggered events waited:~s",
	...
	EventString = case ?getAttr(triggered_events) of
		...
		Events ->
			...
			text_utils:format( "waiting for the completion of ~B world "
							   "events:~s", [ length( Events ), ListString ] )

	end,

	ObjectTypeString = object_table_to_string( State ),

	text_utils:format( "Object manager knowing ~s, ~s and ~s",
...
% (static method)
%
-spec create_default_managers( [ object_manager_def() ],
							   world_manager_pid(), load_balancer_pid() ) ->
									 [ object_manager_pid() ].
create_default_managers( ObjectManagerDefs, WorldManagerPid,
						 LoadBalancerPid ) ->
	create_default_managers( ObjectManagerDefs, WorldManagerPid,
							 LoadBalancerPid,
							 _IdentificationServerPid=undefined ).
...
% process a given event, thus providing a naive load balancing.
%
% Returns the list of their PIDs in the same order as the one of their names.
%
% (static method)
...
create_default_managers( ObjectManagerDefs, WorldManagerPid, LoadBalancerPid,
	...
	% instanciated:
	%
	ManagerCount = erlang:floor( system_utils:get_core_count() * 1.6 ),
	...
	class_Actor:create_initial_actors( ManagerCreationSpecs, LoadBalancerPid ).



% Creates (initially, i.e. before the simulation is started) the specified
% specific (i.e. defined in a class of their own, as opposed to default ones,
% direct instances of this class) object managers, supposing here that they
...
% (static method)
%
-spec create_specific_managers( [ wooper:classname() ], world_manager_pid(),
						load_balancer_pid() ) -> [ object_manager_pid() ].
...
-spec create_initial_objects( object_manager_pid(), dataflow_object_type(),
			dataflow_pid(), [ construction_parameters() ] ) -> [ object_pid() ].
...
% Creates, at runtime (i.e. in the course of the simulation), a object of
...
%
% To be called from an actor, typically a specialised object manager.
%
% (helper)
%
...
create_runtime_object( ObjectType, DataflowPid, CoreConstructionParameters,
	...
	?void_fmt( "Creating a runtime instance of object type '~s', associated "
				"to dataflow ~w, based on following core construction "
				"parameters: ~p.",
				[ ObjectType, DataflowPid, CoreConstructionParameters ] ),
...
% (helper)
%
-spec get_object_pids( [ external_id() ], wooper:state() ) ->
							 [ object_pid() ].
get_object_pids( ExternalIdentifiers, State ) ->
	...
	wooper:execute_request( getBlockPIDs, [ ExternalIdentifiers ],
