...
%
% During a tick, a given actor may be scheduled up to once.
%
% Most containers of actor PIDs are sets (hence never containing duplicates), as
% generally we do not want to send a message more than once to an actor, to
% minimise the sendings; moreover sets allow for faster look-ups.
...
%  - converting a duration in wall-clock seconds into a difference of ticks
...
% moment labelled {17,0} would be introduced. Next one would be {17,1} and so
% on. If diascas had to be inserted in turn between these last two diascas, then
% {17,0,0}, {17,0,1}, etc. would be introduced.
%
%
...
%
-type count_info() :: { actor_count(), basic_utils:count(),
						basic_utils:count() }.
...
% pop first (leftmost) entry; so we use a custom, plain, ordered list, knowing
% that in most cases the list should not be very long (the agenda is indexed by
% future tick offsets, not by actors); inside this (plain) list of spontaneous
% entries, each element is a {Tick,ActorSet} pair, where ActorSet is a set
% (unordered, with no duplicates) of PIDs (see set_utils); we may have to search
% for a specific PID in it and to iterate on its elements
%
% - on some other lists we just have to iterate; this reason holds for the list
% of actors to be triggered on the next diasca
...
% ordered, not having duplicates), therefore we preferred to use more advanced
...
% case this is the root time manager)
%
% - child_managers :: set_utils:set( time_manager_pid() ) is the set (unordered,
% with no duplicates) of the direct child managers of this manager
%
% - known_local_actors :: set_utils:set( actor_pid() ) is the set of the PIDs of
% all known actors that are directly managed by this time manager; useful to
% notify them for example that the simulation starts (it is a set, as for
% example we have to ensure that an actor is not subscribed yet before
% subscribing it)
%
...
%
% - initial_timestamp :: time_utils:timestamp() is the wallclock timestamp
...
% is a tick offset, and second element is a set of actors, like (the set being
% represented here as a list) in: [ {4,[Pid1,Pid2]}, {7,[Pid3]}, {8,[Pid1]} ];
% these actor sets are not basic lists, even if the most usual action performed
% on them will be to iterate over them, as there will be nevertheless other
% random-access operations performed on them (testing for membership - since, at
% any tick, a given actor (PID) should not be specified more than once, or
% withdrawing spontaneous actions, etc.)
%
...
% 'no_planned_action', to 'new_diasca_needed' or to an actual tick offset to
% keep track of its soonest known deadline
%
% - actors_to_trigger_in_one_diasca :: set_utils:set( actor_pid() ) is the set
...
% the current overall diasca; a local actor is thus listed up to once, even if
% it received more than one actor message on the corresponding diasca (this is
% ensured actor-side, see schedule_trigger_already_sent, and also because the
% time managers stores them in a set, not a list)
%
% - actors_to_trigger_in_two_diascas :: set_utils:set( actor_pid() ) is the set
% of actors that should be triggered on the diasca *after* the next scheduled
% one, due to the race condition described in the implementation notes; a local
% actor must be listed up to once, even if it received more than one actor
% message on the corresponding diasca (this is ensured actor-side, see
% schedule_trigger_already_sent); this is also a set as it will be assigned to
...
% - terminated_actors :: [ actor_pid() ] is a plain list of actors that have
...
% tick); does not seem to be actually used currently (never set)
%
% - terminating_actors :: [ actor_pid() ] is a plain list (not a set: just
% needing to add elements or iterate on them all) of actors that are actively
% terminating (i.e. within a bounded number of diascas) this diasca, but have
% not terminated yet; the list is established (reset and rebuilt) at each diasca
%
% - actors_to_delete_at_next_tick :: [ actor_pid() ] is a list of the PIDs of
% all actors that are terminating and whose (deferred) deletion is to happen at
% next tick
%
% - waited_child_managers :: set_utils:set( time_manager_pid() ) is the set of
% child managers that are still waited, for the current diasca to finish; it is
% a set as well, since a simulation distributed over a cluster may involve, say,
% 300+ nodes, if not 65000 nodes on a Bluegene/Q?
%
% - waited_spontaneous_actors :: set_utils:set( actor_pid() ) is the set of the
% actors whose spontaneous behaviour has been scheduled this tick (as diasca 0)
% and that are still waited, for the current diasca to finish
%
% - waited_triggered_actors :: set_utils:set( actor_pid() ) is the set of the
% actors whose triggered behaviour has been scheduled this diasca and that are
% still waited, for the current diasca to finish
%
...
% that are waited this diasca (spares the need of polling numerous sets each
...
% only used for the root time manager)
%
% - overall_actor_count :: actor_count() is the current total number of actors
% in the simulation, as notified by the load balancer (only useful in the root
% time manager)
%
% - scheduled_tracking :: actor_count() is the count of actors to be
...
construct( State, SimulationTickDuration, InteractivityMode,
	...
	%send_debug_fmt( CategorizedState, "Code path for node '~s' is:~s",
...
destruct( State ) ->
	...
	naming_utils:unregister( get_registration_name(), ?registration_type ),
...
% Must not be called while the simulation is running.
%
% See also: setFinalSimulationDate/3.
%
% (oneway)
%
-spec setInitialSimulationDate( wooper:state(), time_utils:date(),
...
% Note: depends on the current initial tick, as is stored as a tick offset.
%
% See also: setInitialSimulationDate/3.
%
% (oneway)
%
-spec setFinalSimulationDate( wooper:state(), time_utils:date(),
							  time_utils:time() ) -> oneway_return().
setFinalSimulationDate( State, Date, Time ) ->
	...
	case DurationInTicks > 0 of
		...
		true ->
			...
			?info_fmt( "New final simulation tick is ~B "
					   "(corresponding to final date ~s).",
					   [ FinalTick,
						 time_utils:get_textual_timestamp( FinalDate ) ] ),
...
% (oneway)
%
-spec startFor( wooper:state(), unit_utils:any_seconds() ) -> oneway_return().
...
%	check_tick_consistency( NewTickOffset, State ),
	...
	case set_utils:is_empty( ToTriggerNextDiasca ) of
		...
		false ->
			throw( { new_tick_whereas_actors_still_to_trigger, NewTickOffset,
...
notifySpontaneousSubtreeCompletion( State, _SubtreeTickOffset, ChildManagerPid,
	  NextActionInSubtree,
...
notifySpontaneousActionsCompleted( State, _ActorTickOffset, ActorPid,
...
-spec notifySpontaneousWatchdogCompleted( wooper:state(), tick_offset() ) ->
												oneway_return().
notifySpontaneousWatchdogCompleted( State, _WatchdogTickOffset ) ->
...
-spec notifyTriggerSubtreeCompletion( wooper:state(), tick_offset(),
	 diasca(), pid(), next_manager_action(), diasca_tracking_info() ) ->
										   oneway_return().
notifyTriggerSubtreeCompletion( State, _SubtreeTickOffset, _SubtreeDiasca,
...
notifyTriggeredActionsCompleted( State, _ActorTickOffset, _ActorDiasca, ActorPid,
	   _NextActorAction=no_diasca_requested, AddedSpontaneousTicks,
	   WithdrawnSpontaneousTicks ) ->
		...
		CurrentTickOffset ->
			...
			case TriggerDiasca of
				...
				UsuallyExpectedTriggerDiasca ->
					...
					UpdatedDiascaActors = set_utils:addAsNew( ActorToSchedule,
													  CurrentDiascaActors ),
...
onSimulationStallDetected( State ) ->
	...
	TriggeredMessage = case TriggeredActorCount of
		...
		TriggeredCount when TriggeredCount > ?too_many_entries ->
			io_lib:format( "still waiting for ~B triggered actors",
						   [ TriggeredCount ] );
...
%
-spec watchdog_main_loop( time_manager_pid(), unit_utils:milliseconds(),
		  unit_utils:milliseconds(), unit_utils:milliseconds(),
...
watchdog_main_loop( TimeManagerPid, DelayBeforeFirstStall,
					...
					NextPeriod, AccumulatedDuration ) ->

	?display_console( "Watchdog ~w running (first stall delay: ~B ms, "
					  "then ~B ms; shutdown delay: ~B ms, next period: ~B ms,"
					  "accumulated: ~B ms).~n",
					  [ self(), DelayBeforeFirstStall,
	...
	after NextPeriod ->
		...
		case AccumulatedDuration of
			...
			TooLong when TooLong > DelayBeforeFailed ->
				...
				DelayString = text_utils:duration_to_string(
								DelayBeforeFailed ),

				?notify_fatal_fmt_cat(
				   "Warning: watchdog on node '~s' detected a too long "
				   "inter-diasca duration (more than ~s), this node is thus "
				   "deemed orphaned, shutting it down now at ~s.~n",
...
%
-spec time_tracker_start( unit_utils:seconds() | 'undefined',
							  unit_utils:seconds() | 'not_undefined',
							  { timing_info(), count_info() },
							  pid(), pid() ) -> no_return().
time_tracker_start( LatestDisplayedSecond, LatestMeasuredSecond,
	...
	time_tracker_main_loop( LatestDisplayedSecond, LatestMeasuredSecond,
							LatestTopInfo={ Timings, Counts }, LoadBalancerPid,
							RootTimeManagerPid, _WaitTime=0 ).
...
-spec time_tracker_main_loop( unit_utils:seconds() | 'undefined',
							  unit_utils:seconds() | 'not_undefined',
							  { timing_info(), count_info() },
							  pid(), pid(), unit_utils:milliseconds() ) ->
									no_return().
time_tracker_main_loop( LatestDisplayedSecond, LatestMeasuredSecond,
						LatestTopInfo={ Timings, Counts }, LoadBalancerPid,
						RootTimeManagerPid, WaitTime ) ->
	...
	receive
		...
		{ NewMeasuredSecond, NewLatestTopInfo } ->
			...
			time_tracker_main_loop( LatestDisplayedSecond, NewMeasuredSecond,
									NewLatestTopInfo, LoadBalancerPid,
									RootTimeManagerPid, _WaitTime=5000 )
	...
	after WaitTime ->
			...
			case LatestMeasuredSecond of
				...
				LatestDisplayedSecond ->
					...
					display_diasca_in_progress(),

					% Reduces a bit (5 seconds) the rhythm (we just show that
...
display_diasca_in_progress() ->
	...
	BaseText = text_utils:format( "diasca still in progress at ~s",
								  [ time_utils:get_textual_timestamp() ] ),
...
% (helper)
%
-spec get_simulation_time_and_date( wooper:state() ) -> time_utils:timestamp().
...
% (helper)
%
-spec timestamp_to_ticks( time_utils:timestamp(), wooper:state() ) -> tick().
...
init( State ) ->
	...
	StopString = case ?getAttr(stop_tick_offset) of
		...
		StopOffset ->
			...
			io_lib:format( "Simulation will stop no later than tick ~B "
						   "(i.e. after ~B ticks, which is a duration of "
						   "~s in virtual time).",
						   [ AbsoluteStop, StopOffset, TextDuration ] )
...
%
-spec start_actors_by_chunks( set_utils:set( actor_pid() ), term() ) ->
									actor_count().
start_actors_by_chunks( InitialActors, StartMessage ) ->

	ExpectedCount = set_utils:size( InitialActors ),

	?display_console( "Starting ~B actors by chunks of ~B.~n",
...
start_actors_by_chunks( InitialActorsIterator, WaitedSet,
						_MaxWaitedCount=?chunk_size, StartMessage,
						TotalCount ) ->
...
wait_for_start_acknowlegments( WaitedType, WaitedList ) ->
	...
	?display_console( "Waiting for start acknowlegments for ~B processes of "
					  "type ~p.~n",
					  [ set_utils:size( WaitedList ), WaitedType ] ),
		...
		false ->
			receive
				...
				{ wooper_result, { WaitedType, Pid } } ->
					...
					DelList = set_utils:safe_delete( Pid, WaitedList ),
		...
		false ->
			...
			receive
				...
				{ wooper_result, { stopped, ManagerPid } } ->
					...
					RemainingManagers = set_utils:safe_delete( ManagerPid,
															   ChildManagers ),
			...
			after WaitDuration ->
				...
				?error_fmt( "Following child time managers failed to report "
							"on time (after ~B milliseconds) that they "
							"stopped: ~p.",
							[ WaitDuration,
							  set_utils:to_list( ChildManagers ) ] )
...
manage_possible_end_of_diasca( State ) ->
	...
	case is_current_diasca_over( State ) of
		...
		false ->
			...
			?display_console( "Time manager ~w still having to wait.~n",
							  [ self() ] ),
...
% (helper)
%
record_progress_message( _Header, _ChildManagerCount, _ScheduledCount,
						 State ) ->
	...
	{ ActualTickOffset, ActualDiasca } = case ?getAttr(previous_timestamp) of
		...
		undefined ->
			% We just started here, let's use closest meaningful timestamp:
	...
	% hence are useful)
	%
	{ _DetailedTopMessage, CompactTopMessageElements, Second } =
...
withdraw_from_agenda( ActorPid, _WithdrawnSpontaneousTicks=[ Tick | T ],
					  Agenda ) ->
...
add_to_agenda( ActorPid, TickOffset, _Agenda=[ { TickOffset, ActorSet } | T ],
			   Acc ) ->
...
launch_watchdog( State ) ->
	...
	?debug_fmt( "Watchdog ~w created and running, with a time-out duration "
				"of ~B ms for any initial stall, then ~B ms for next ones, "
				"and of ~B ms for automatic shutdowns; "
				"notifying now initial actors that the simulation started.",
				[ WatchdogPid, DelayBeforeFirstStall, DelayBetweenNextStalls,
				  DelayBeforeFailed ] ),
...
% time managers.
%
-spec get_maximum_teardown_duration() -> unit_utils:milliseconds().
...
get_max_inter_diasca_duration_until_failure_triggered() ->
	...
	?display_console( "(in production mode, thus ~w will be using "
					  "extended time-outs for simulation progress)~n",
					  [ self() ] ),
...
get_max_inter_diasca_duration_until_failure_triggered() ->
	% 2 hours already quite comfortable:
	2 * 60 * 60 * 1000.
